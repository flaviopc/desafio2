...
import com.dockbank.bank.api.exceptionhandler.MessageException.Problema;
...
public class APIExceptionHandler extends ResponseEntityExceptionHandler {
        ...
        protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
                ...
                List<MessageException.Problema> problemObjects = extrairErrosDosCampos(bindingResult);
                MessageException messageException = MessageException.builder()
        ...
        private List<Problema> extrairErrosDosCampos(BindingResult bindingResult) {
                return bindingResult.getAllErrors().stream()
                                .map(objectError -> {
                                        String mensagem = objectError.getDefaultMessage();

                                        String nomeObject = objectError.getObjectName();

                                        if (objectError instanceof FieldError)
                                                nomeObject = ((FieldError) objectError).getField();
